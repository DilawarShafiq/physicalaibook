...
sequenceDiagram
  ...
  participant Net as Availity (REST → EDI)
  participant Payer
  Agent->>Net: 270 inquiry (NPI, member ID, DOB, service type)
  Net->>Payer: route X12 270
  Payer-->>Net: X12 271 (coverage, deductible, copay, COB)
  Net-->>Agent: 271 response
  Note over Agent: parse → active? deductible left? PA required?
```
