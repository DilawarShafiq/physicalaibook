...
This matters most in exactly the multi-agent, high-volume design you built in the previous lesson, where sub-agents fire repeatedly against the same payer policies and codebooks. Cache reads are billed at roughly a tenth of the base input price, and Anthropic reports up to ~90% cost and ~85% latency reductions for long prompts — which is why a healthy cache hit rate, above 80%, should be an explicit operational target, not an afterthought. Caching also extends to tool definitions, so the agent's stable tool inventory rides along in the cached prefix.
