...
```mermaid
flowchart LR
    A["Full clinical record"] --> B["LLM candidate coder"]
    B --> C["Suggested codes with text spans"]
    C --> D["Validate against ICD-10 tabular list"]
    D -->|"Valid"| E["Human coder sign-off"]
    D -->|"Invalid or hallucinated"| F["Drop or flag"]
    E --> G["Billable result"]
```
